...
## Boldness
Time spent in the middle is our measure of boldness. The variables are the proportion of time spent in the middle (out of trial time = 360 sec) or the actual number of seconds spent in the middle zone. It is difficult to find a well-fitting model family or transformation of the proportion data.
...
# I tried binomial, poisson and negative binomial. Negative binomial is the only model that passed all model assumption checks.
simulationOutput <- simulateResiduals(fittedModel = lmer(log(CumDurZ2+1) ~ Treatment + scale(Age, center = T, scale = T) + Treatment*scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity), data=etho), n=250) #model failed to converge with interaction term
...
# Now which variables to include?
b1 = glmer.nb(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + Treatment*scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity), data=etho) # full model
b1 = lmer(log(CumDurZ2+1) ~ Treatment + scale(Age, center = T, scale = T) + Treatment*scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity), data=etho)
b2 = glmer.nb(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity), data=etho) # no interaction
b2 = glmer(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity) + (1|Trial), family = poisson, data=etho)
b2 = lmer(log(CumDurZ2+1) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity), data=etho)
anova(b1,b2) # no difference, so leave out the interaction (proceed with model 2)
# For both of these models, I get the warning about it being a singular fit. Used Ben Bolker's code below, and random effect 3 (Paternity) accounts for 0 or little variance.
theta <- getME(b1, "theta")
diag.element <- getME(b1, "lower")==0
which(theta[diag.element]<3e-5)

b3 = glmer.nb(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch), data=etho) # no interaction, no Paternity
b3 = glmer(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Trial), family = poisson, data=etho)
b3 = lmer(log(CumDurZ2+1) ~ Treatment + scale(Age, center = T, scale = T) + Treatment*scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch), data=etho)
# no longer getting warning about singular fit
anova(b2,b3) # no difference, proceed with model 3

b4 = glmer.nb(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName), data=etho) # only Fish ID random effect
b4 = glmer(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName) + (1|Trial), family = poisson, data=etho)
b4 = lmer(log(CumDurZ2+1) ~ Treatment + scale(Age, center = T, scale = T) + Treatment*scale(Age, center = T, scale = T) + (1|FishName), data=etho)
anova(b3,b4) # also no difference, so proceed with model that only has the random effect of FishName 
library(ggpubr)
ggqqplot(resid(b4))
plot(resid(b4))
hist(resid(b4))

### We can't use negative binomial regression in the rptR function. We can use Poisson regression if we include the observation-level random effect to remedy the overdispersion.
etho$scale.age = scale(etho$Age, scale = T, center = T)
etho$Trial = as.character(etho$Trial)
etho$FishName = as.character(etho$FishName)
rptB = rpt(log(CumDurZ2+1) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName), datatype = "Gaussian", data=etho, grname = "FishName", nboot = 500, npermut = 500)
rptB

ggplot(etho[which(etho$Treatment == "0"),], aes(x = FishName, y = log(CumDurZ2+1))) +
  #facet_wrap(~Treatment) +
  geom_boxplot() +
  theme_classic()
