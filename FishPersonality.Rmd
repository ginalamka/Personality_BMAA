...
rpt.eclong <- ec.long$VCV[,"FishName"]/(ec.long$VCV[,"FishName"]+ec.short$VCV[,"units"])
p1 <- hist(rpt.ecshort)                     
p2 <- hist(rpt.eclong)                     
plot( p1, col=rgb(0,0,1,1/4), xlim=c(0,0.3))  
plot( p2, col=rgb(1,0,0,1/4), xlim=c(0,0.4), add=T)
# Posterior predicted values for exploration, control treatment fish are overlapping at short and long terms

#low
rpt.elshort <- el.short$VCV[,"FishName"]/(el.short$VCV[,"FishName"]+el.short$VCV[,"units"])
rpt.ellong <- el.long$VCV[,"FishName"]/(el.long$VCV[,"FishName"]+el.short$VCV[,"units"])
p1 <- hist(rpt.elshort)                     
p2 <- hist(rpt.ellong)                     
plot( p1, col=rgb(0,0,1,1/4), xlim=c(0,0.3))  
plot( p2, col=rgb(1,0,0,1/4), xlim=c(0,0.4), add=T)
low = data.frame(cbind(sort(rpt.elshort),sort(rpt.ellong)))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff)
1729/2900 # 60% of short rpt values are less than long rpt values, if I sort each vector than almost 100% of short values are less than long values

#high
rpt.ehshort <- eh.short$VCV[,"FishName"]/(eh.short$VCV[,"FishName"]+eh.short$VCV[,"units"])
rpt.ehlong <- eh.long$VCV[,"FishName"]/(eh.long$VCV[,"FishName"]+eh.short$VCV[,"units"])
p1 <- hist(rpt.ehshort)                     
p2 <- hist(rpt.ehlong)                     
plot( p1, col=rgb(0,0,1,1/4), xlim=c(0,0.3))  
plot( p2, col=rgb(1,0,0,1/4), xlim=c(0,0.4), add=T)
...
table(high$diff) # 2748 0s, 152 1s
2748/2900 #95% of short data are less than long data, if I sort short/long vectors than 100% of matched pairs the short is less than the long.
...
mean(rpt.bclong)
HPDinterval(rpt.bclong)

cont = data.frame(cbind(sort(rpt.bcshort),sort(rpt.bclong)))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
# 100% of short rpt values less than long values
...
low = data.frame(cbind(sort(rpt.blshort),sort(rpt.bllong)))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff)
# 100% of short rpt values less than long values
...
high = data.frame(cbind(sort(rpt.bhshort),sort(rpt.bhlong)))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) # >99% of short values are less than long values. p = 0.001
...
p1 <- hist(rpt.acshort)                     
p2 <- hist(rpt.aclong)                     
plot( p1, col=rgb(0,0,1,1/4), xlim=c(0,0.6))  
plot( p2, col=rgb(1,0,0,1/4), xlim=c(0,0.6), add=T) # distributions barely overlapping
cont = data.frame(cbind(rpt.acshort,rpt.aclong))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff) # almost 100% of short values are lower than long values, with and without sorting the vectors
...
p2 <- hist(rpt.allong)                     
plot( p1, col=rgb(0,0,1,1/4), xlim=c(0,0.6))  
plot( p2, col=rgb(1,0,0,1/4), xlim=c(0,0.6), add=T) # distributions moderately overlapping
low = data.frame(cbind(rpt.alshort,rpt.allong))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff) # 94% of short values less than long values, 100% if sorted vectors
...
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) # 100% of short values less than long values, regardless if vectors are sorted
