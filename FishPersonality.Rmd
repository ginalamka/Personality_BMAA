...
Time spent in the middle can be the proportion of time spent in the middle (out of trial time = 360 sec; "CumDur.Z2") or the actual number of seconds spent in the middle zone ("CumDurZ2"). Negative binomial fits well to the proportion data. A poisson model with an observation-level random effect fits well to the count data.
...
#Negative Binomial:
simulationOutput <- simulateResiduals(fittedModel = glmer.nb(round(CumDur.Z2) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch), data=etho, control=glmerControl(optimizer="bobyqa",
...
simulationOutput <- simulateResiduals(fittedModel = glmer(round(CumDur.Z2) ~ Treatment + (scale.age|FishName) + (1|OLRE), family = "poisson", data=etho), n=500) #isSingular warning because of Paternity & Clutch
...
e1 = glmer(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age + (scale.age|FishName) + (1|Clutch) + (1|Paternity) + (1|OLRE), family = "poisson", data=etho, control=glmerControl(optimizer="bobyqa", #add optimizer because otherwise it fails to converge
                                 optCtrl=list(maxfun=2e5))) # full model
theta <- getME(e1, "theta")
diag.element <- getME(e1, "lower")==0
which(theta[diag.element]<3e-5)
#Fit is singular because of clutch and paternity

e2 = glmer(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age + (scale.age|FishName) + (1|OLRE), family = "poisson", data=etho,
           control=glmerControl(optimizer="bobyqa", #add optimizer because otherwise it fails to converge
                                 optCtrl=list(maxfun=2e5)))
anova(e1,e2) # no difference, proceed with model 2

e3 = glmer(round(CumDur.Z2) ~ Treatment + scale.age + (scale.age|FishName) + (1|OLRE), family = "poisson", data=etho) # no random slope
anova(e2,e3) # no difference, proceed with simpler model e3 (no random slope).

e4 = glmer(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName) + (1|OLRE), family = "poisson", data=etho) 
anova(e3,e4) # e4 with treatment by age interaction accounts for more variance

e45 = glmer(round(CumDur.Z2) ~ Treatment + scale.age + (1|FishName) + (1|OLRE), family = "poisson", data=etho) 
anova(e3,e4)

#### rptR package for repeatability estimate
library(rptR)
rptE = rpt(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName), datatype = "Poisson", data=etho, grname = "FishName", nboot = 200, npermut = 100) # Poisson models are automatically fit with an OLRE in the rptR package.
rptE # R = 0.10 but se = 0 and CI is 0.10-0.10 so something is wrong here.


#### MCMCglmm instead of rptR for repeatability estimate. MCMC with Poisson family automatically incorporates an OLRE
...
library(MCMCglmm)
prior = list(R=list(R1=list(V=1,nu=0.002),
                    R2=list(V=1,nu=0.002),
                    R3=list(V=1,nu=0.002)), 
             G=list(G1=list(V=1,nu=0.002))) #weak priors
...
##### Takes a loooong time to run with 1000. 10 permutations takes ~10 min; 50 takes ~45min
results = rep(NA, 500) 
for(i in 1:50){
...
hist(results,xlim = c(0,0.13),xlab = "Repeatability estimates from randomized data") # average repeatability estimate from each iteration of the permutation, which randomizes choices within reversals
abline(v=0.12,col="red")
sum(results > 0.12)/50 # probability the repeatability estimate from our actual data is greater than the repeatability estimates generated from the permutations which randomized the data
# no estimates >= to 0.12. p = 0
...
# visualize fish change in exploration over time as a function of treatment
library(ggplot2)
library(ggbreak)
p <- ggplot(etho, aes(x = as.factor(Age), y = log(CumDur.Z2+1), group = FishName, colour = Treatment)) +
  geom_line(alpha = 0.2,linewidth = 2) +
  stat_summary(aes(group = Treatment), geom = "point", fun = mean,
    shape = 17, size = 3) +
  theme_bw()
...
# interaction between treatment 5 and age is significant at B = -0.23, p = 0.04
library(ggplot2)
library(tidyr)
library(dplyr)
...
etho.red = etho %>%
  ...
  summarise(mean = mean(CumDur.Z2), sd = sd(CumDur.Z2))
# plot to visualize interaction effect
exp.plot = ggplot(etho.red, aes(x = as.numeric(Age), y = mean, color = Treatment))+
  geom_pointrange(aes(ymin = mean-sd, ymax = mean+sd),position = position_dodge(width = 7),alpha = 0.6) +
  geom_smooth(method = "glm",se = F,linewidth = 1) +
  theme_classic() + 
  scale_color_manual(
    values = c("red","darkgray","black"),
    labels = c("0" = "Control",
               "5" = "Low",
               "25" = "High")) +
  scale_x_continuous(
 #   labels=c("14","21","49","77","105","133","161","189"),
    breaks = c(14,21,49,77,105,133,161,189))+
  theme(
    axis.title = element_text(size = 18),
    axis.text = element_text(size = 16)) +
  ylab("Proportion of time exploring") + xlab("Age (days)") + labs(color = "Treatment condition")
exp.plot
```
...
b1 = coxme(Surv(LatencyZ2360,event)~Treatment + scale.age + Treatment*scale.age + (scale.age|FishName), data = etho) # coxme_icc function can only handle 1 random effect
b2 = coxme(Surv(LatencyZ2360,event)~Treatment + scale.age + (scale.age|FishName), data = etho)
anova(b1,b2) # no difference, exclude interaction term
b3 = coxme(Surv(LatencyZ2360,event)~Treatment + scale.age + (1|FishName), data = etho)
anova(b2,b3) # random slope significantly adds to variance explained

# Takes a while to run: coxme(Surv(LatencyZ2360,event)~Treatment + scale.age + (scale.age|FishName)
coxme_pval(b3,etho,boot=100) #  p = 0.01
coxme_icc_ci(b3, upper.multiplier = 10) # ICC = 0.04 (0.01-0.08)
...
#Poisson 
simulationOutput <- simulateResiduals(fittedModel = glmer(round(LatencyZ2360) ~ Treatment + scale.age + Treatment*scale.age + (scale.age|FishName) + (1|OLRE), family = poisson, data=etho,control=glmerControl(optimizer="bobyqa", #add optimizer because otherwise it fails to converge
                                 optCtrl=list(maxfun=2e5))), n=500) 
#Gaussian
simulationOutput <- simulateResiduals(fittedModel = lmer(log(LatencyZ2360+1) ~ Treatment + scale.age + Treatment*scale.age + (scale.age|FishName) + (1|Clutch) + (1|Paternity), data=etho), n=500)
...
l1 = lmer(log(LatencyZ2360+1) ~ Treatment + scale.age + Treatment*scale.age + (scale.age|FishName) + (1|Clutch) + (1|Paternity), data=etho) #full model
l2 = lmer(log(LatencyZ2360+1) ~ Treatment + scale.age + Treatment*scale.age + (scale.age|FishName) + (1|Clutch), data=etho)
anova(l1,l2) #no difference, use simpler model (l2)
VarCorr(l2) #Random effects and residual variance

l3 = lmer(log(LatencyZ2360+1) ~ Treatment + scale.age + Treatment*scale.age + (scale.age|FishName), data=etho)
anova(l2,l3) #l2 is better

l4 = lmer(log(LatencyZ2360+1) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch), data=etho)
...
l6 = lmer(log(LatencyZ2360+1) ~ Treatment + scale.age + (1|FishName) + (1|Clutch), data=etho)
anova(l4,l6) #l4 significantly better, random slope adds to variance explained

l.rpt = rpt(log(LatencyZ2360+1) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch), datatype = "Gaussian", data=etho, grname = "FishName", nboot = 500, npermut = 500) #singular fit warning with or without Clutch
l.rpt
# R = 0.10 (0.06-0.16)
...
#random slope plot
b <- ggplot(etho, aes(x = as.factor(Age), y = log(LatencyZ2360+1), group = FishName, colour = Treatment)) +
...
summary(b2) # survival model
# No effect of treatment. Significant effect of age ß = 0.24, p < 0.05
nlme::VarCorr(b2)
# fish ID and age R = 0.07
...
bsurv.plot = ggsurvplot(b.plot.sfit, data = etho, fun = 'event', 
                       risk.table = F, pval = F, palette = c("red","#999999","black"), 
                       xlab = "Latency to enter the risky area (sec)", 
                       ylab = "Proportion of fish", size = 1.5, 
                       font.x = 16, font.y = 16,
                       legend = c(0.7,0.2),legend.title = "Treatment condition",legend.labs = c("Control","Low","High"))

b.plot.sfit2 = survfit(Surv(LatencyZ2360, event)~as.numeric(Age), data = etho)
bsurv.plot2 = ggsurvplot(b.plot.sfit2, data = etho, fun = 'event', 
                       risk.table = F, pval = F,  
                       xlab = "Latency to enter the risky area (sec)", 
                       ylab = "Proportion of fish", size = 1.5, 
                       font.x = 16, font.y = 16, font.legend = 16,
                       legend = c(0.7,0.3), legend.title = "Age (days)",legend.labs = c(14,21,49,77,105,133,161,189),
                       palette = "grey")
# As age increases, latency to enter the center generally decreases
...
#Gaussian
simulationOutput <- simulateResiduals(fittedModel = lmer(log(Mean.Mobility+1) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch), data=etho), n=500)
#Poisson 
simulationOutput <- simulateResiduals(fittedModel = glmer(round(Mean.Mobility) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch) + (1|OLRE), family = poisson, data=etho), n=500) 
...
m1 = lmer(log(Mean.Mobility+1) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch) + (1|Paternity), data=etho) #full model
m2 = lmer(log(Mean.Mobility+1) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch), data=etho)
anova(m1,m2) #no difference, use simpler model (m2)

m3 = lmer(log(Mean.Mobility+1) ~ Treatment + scale.age + (scale.age|FishName), data=etho)
anova(m2,m3) #m2 is better

m4 = lmer(log(Mean.Mobility+1) ~ Treatment + scale.age + (1|FishName) + (1|Clutch), data=etho)
anova(m2,m4) #m2 is better, significant improvement with random slope
...
anova(m5,m2) #m2 is better - accounts for somethin!


m.rpt = rpt(log(Mean.Mobility+1) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch), datatype = "Gaussian", data=etho, grname = c("FishName","Clutch"), nboot = 500, npermut = 500) #singular fit warning with or without Clutch
m.rpt
# R = 0.13 (0.07-0.18)
# p.LRT and p.permute < 0.01
...
#random slope plot
a <- ggplot(etho, aes(x = as.factor(Age), y = log(Mean.Mobility+1), group = FishName, colour = Treatment)) +
  geom_line(alpha = 0.2,size = 2) +
  stat_summary(aes(group = Treatment), geom = "point", fun = mean,
    shape = 17, size = 3) +
  theme_bw()
...
m2 = lmerTest::lmer(log(Mean.Mobility+1) ~ Treatment + scale.age + (scale.age|FishName) + (1|Clutch), data=etho)
summary(m2)
anova(m2) # global effect of Treatment and age

library(emmeans)
c1 = emmeans(m2, pairwise ~ Treatment,type="response") 
plot(c1$emmeans) 
c1$contrasts %>%
     summary(infer = TRUE)
...
etho.red = etho %>%
  group_by(Age, Treatment) %>%
  summarise(mean = mean(Mean.Mobility), sd = sd(Mean.Mobility))
# plot to visualize treatment effect
act.plot = ggplot(etho.red, aes(x = as.numeric(Age), y = mean, color = Treatment))+
  geom_pointrange(aes(ymin = mean-sd, ymax = mean+sd),position = position_dodge(width = 7),alpha = 0.6) +
  geom_smooth(method = "lm",se = F) +
  theme_classic() +
  scale_color_manual("Treatment condition",
    values = c("red","darkgray","black"),
    labels = c("0" = "Control",
               "5" = "Low",
               "25" = "High")) +
  ...
  theme(
    axis.title = element_text(size = 18),
    axis.text = element_text(size = 16)) +
  ylab("Percent time active") + xlab("Age (days)")
```
...
## Evidence for acute or chronic effects of BMAA?
Splitting data into early (days 14 - 77) and late trials (days 105 - 189) to test whether BMAA affected behavior during and shortly after exposure, compared to longer-term post exposure. In other words, maybe there was an acute impact of BMAA on behavior but no developmental effects causing long-term changes.
```{r short-term effects}
etho$term = ifelse(etho$Age == 14 | etho$Age == 21 | etho$Age == 49 | etho$Age == 77, "short", "long")
etho$term2 = ifelse(etho$Age == 14 | etho$Age == 21 | etho$Age == 49, "short","long")
etho$term3 = ifelse(etho$Age == 14 | etho$Age == 21, "short","long")

## Repeatability by term with Treatments combined
#exploration
library(MCMCglmm)
prior = list(R=list(R1=list(V=1,nu=0.002),
                    R2=list(V=1,nu=0.002),
                    R3=list(V=1,nu=0.002)), 
             G=list(G1=list(V=1,nu=0.002))) #weak priors
e.short = MCMCglmm(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term == "short"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- e.short$VCV[,"FishName"]/(e.short$VCV[,"FishName"]+e.short$VCV[,"units"]) 
mean(repeata) # Repeatability: 
HPDinterval(repeata, 0.95) 

e.long = MCMCglmm(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term == "long"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- e.long$VCV[,"FishName"]/(e.long$VCV[,"FishName"]+e.long$VCV[,"units"]) 
mean(repeata) # Repeatability: 
HPDinterval(repeata, 0.95)
...
library(coxme)
etho$event = ifelse(etho$LatencyZ2360 == 360,0,1)
b.short = coxme(Surv(LatencyZ2360,event)~Treatment + scale.age + (1|FishName), data = etho[which(etho$term == "short"),]) # coxme_icc function can only handle 1 random effect

b.long = coxme(Surv(LatencyZ2360,event)~Treatment + scale.age + (1|FishName), data = etho[which(etho$term == "long"),]) 

# Takes a while to run: coxme(Surv(LatencyZ2360,event)~Treatment + scale.age + (scale.age|FishName)
coxme_pval(b.short,etho[which(etho$term == "short"),],boot=100)#  p > 0.05 
coxme_pval(b.long,etho[which(etho$term == "long"),],boot=100) # p < 0.05

coxme_icc_ci(b.short, upper.multiplier = 10) # ICC = 
coxme_icc_ci(b.long, upper.multiplier = 10)


#activity
a.short = rpt(log(Mean.Mobility+1) ~ Treatment + scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "short"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
a.short

a.long = rpt(log(Mean.Mobility+1) ~ Treatment + scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "long"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
a.long

tdata = data.frame("trait" = c("exploration","exploration","boldness","boldness",
                              "activity","activity"),
                  "Repeatability" = c(0.06, 0.18, 0.04, 0.21, 0,0.28),
                  "UCI" = c(0.16, 0.29, 0.10, 0.29, 0.1,0.38),
                  "LCI" = c(0, 0.07, 0.0, 0.14, 0,0.17),
                  "term" = c("short","long","short","long","short","long")
                  )
tdata$trait = as.factor(tdata$trait)
term.plot = ggplot(tdata, aes(x = trait, y = Repeatability, color = term))+
  geom_pointrange(aes(ymin = LCI, ymax = UCI)) +
  theme_classic() +
  theme(
    axis.title = element_text(size = 18),
    axis.text = element_text(size = 16)) +
  ylab("Repeatability") + xlab("Personality trait")

### Repeatability of all fish combined is much lower in the short term compared to long term.

##### What about broken out by treatment? ######
#exploration
library(MCMCglmm)
prior = list(R=list(R1=list(V=1,nu=0.002),
                    R2=list(V=1,nu=0.002),
                    R3=list(V=1,nu=0.002)), 
             G=list(G1=list(V=1,nu=0.002))) #weak priors
el.short = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- el.short$VCV[,"FishName"]/(el.short$VCV[,"FishName"]+el.short$VCV[,"units"]) 
mean(repeata)  # R = 0.05
HPDinterval(repeata, 0.95) # CI = 0.0001-0.19
el.short2 = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term2 == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- el.short2$VCV[,"FishName"]/(el.short2$VCV[,"FishName"]+el.short2$VCV[,"units"]) 
mean(repeata)  # R = 0.13
HPDinterval(repeata, 0.95) # CI = 0.0002-0.36
el.short3 = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term3 == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- el.short3$VCV[,"FishName"]/(el.short3$VCV[,"FishName"]+el.short3$VCV[,"units"]) 
mean(repeata)  # R = 0.07
HPDinterval(repeata, 0.95) # CI = 0.0002-0.30

eh.short = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- eh.short$VCV[,"FishName"]/(eh.short$VCV[,"FishName"]+eh.short$VCV[,"units"]) 
mean(repeata)  # R = 0.03
HPDinterval(repeata, 0.95) # CI = 0-0.14
eh.short2 = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term2 == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- eh.short2$VCV[,"FishName"]/(eh.short2$VCV[,"FishName"]+eh.short2$VCV[,"units"]) 
mean(repeata) # R = 0.08 
HPDinterval(repeata, 0.95) # CI = 0.0001-0.28
eh.short3 = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term3 == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- eh.short3$VCV[,"FishName"]/(eh.short3$VCV[,"FishName"]+eh.short3$VCV[,"units"]) 
mean(repeata)  # R = 0.26
HPDinterval(repeata, 0.95) # CI = 0-0.57

ec.short = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- ec.short$VCV[,"FishName"]/(ec.short$VCV[,"FishName"]+ec.short$VCV[,"units"]) 
mean(repeata) # R = 0.12 
HPDinterval(repeata, 0.95) 
ec.short2 = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term2 == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- ec.short2$VCV[,"FishName"]/(ec.short2$VCV[,"FishName"]+ec.short2$VCV[,"units"]) 
mean(repeata)  
HPDinterval(repeata, 0.95) 
ec.short3 = MCMCglmm(round(CumDur.Z2) ~ scale.age, random = ~FishName, family = "poisson", data = etho[which(etho$term3 == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- ec.short3$VCV[,"FishName"]/(ec.short3$VCV[,"FishName"]+ec.short3$VCV[,"units"]) 
mean(repeata)  
HPDinterval(repeata, 0.95) 

el.long = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- el.long$VCV[,"FishName"]/(el.long$VCV[,"FishName"]+el.long$VCV[,"units"]) 
mean(repeata) # R = 0.05
HPDinterval(repeata, 0.95)
el.long2 = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term2 == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- el.long2$VCV[,"FishName"]/(el.long2$VCV[,"FishName"]+el.long2$VCV[,"units"]) 
mean(repeata) # R 
HPDinterval(repeata, 0.95)
el.long3 = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term3 == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- el.long3$VCV[,"FishName"]/(el.long3$VCV[,"FishName"]+el.long3$VCV[,"units"]) 
mean(repeata) # R 
HPDinterval(repeata, 0.95)

eh.long = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- eh.long$VCV[,"FishName"]/(eh.long$VCV[,"FishName"]+eh.long$VCV[,"units"]) 
mean(repeata) # R = 0.27
HPDinterval(repeata, 0.95)
eh.long2 = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term2 == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- eh.long2$VCV[,"FishName"]/(eh.long2$VCV[,"FishName"]+eh.long2$VCV[,"units"]) 
mean(repeata) # R =0.21 
HPDinterval(repeata, 0.95)
eh.long3 = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term3 == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- eh.long3$VCV[,"FishName"]/(eh.long3$VCV[,"FishName"]+eh.long3$VCV[,"units"]) 
mean(repeata) # R = 0.12
HPDinterval(repeata, 0.95)

ec.long = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- ec.long$VCV[,"FishName"]/(ec.long$VCV[,"FishName"]+ec.long$VCV[,"units"]) 
mean(repeata) # R = 0.13 
HPDinterval(repeata, 0.95)
ec.long2 = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term2 == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- ec.long2$VCV[,"FishName"]/(ec.long2$VCV[,"FishName"]+ec.long2$VCV[,"units"]) 
mean(repeata) # R = 0.12
HPDinterval(repeata, 0.95)
ec.long3 = MCMCglmm(round(CumDur.Z2) ~ scale.age , random = ~FishName, family = "poisson", data = etho[which(etho$term3 == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
repeata <- ec.long3$VCV[,"FishName"]/(ec.long3$VCV[,"FishName"]+ec.long3$VCV[,"units"]) 
mean(repeata) # R = 0.14
HPDinterval(repeata, 0.95)
...
library(coxme)
etho$event = ifelse(etho$LatencyZ2360 == 360,0,1)
bc.short = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term == "short" & etho$Treatment == "0"),]) 
bc.short2 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term2 == "short" & etho$Treatment == "0"),]) 
bc.short3 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term3 == "short" & etho$Treatment == "0"),]) 

bl.short = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term == "short" & etho$Treatment == "5"),])
bl.short2 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term2 == "short" & etho$Treatment == "5"),])
bl.short3 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term3 == "short" & etho$Treatment == "5"),])

bh.short = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term == "short" & etho$Treatment == "25"),])
bh.short2 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term2 == "short" & etho$Treatment == "25"),])
bh.short3 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term3 == "short" & etho$Treatment == "25"),])


bc.long = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term == "long" & etho$Treatment == "0"),])
bc.long2 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term2 == "long" & etho$Treatment == "0"),])
bc.long3 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term3 == "long" & etho$Treatment == "0"),])

bl.long = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term == "long" & etho$Treatment == "5"),])
bl.long2 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term2 == "long" & etho$Treatment == "5"),])
bl.long3 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term3 == "long" & etho$Treatment == "5"),])

bh.long = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term == "long" & etho$Treatment == "25"),])
bh.long2 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term2 == "long" & etho$Treatment == "25"),])
bh.long3 = coxme(Surv(LatencyZ2360,event)~scale.age + (1|FishName), 
                 data = etho[which(etho$term3 == "long" & etho$Treatment == "25"),])

coxme_icc_ci(bc.short, upper.multiplier = 10) 
coxme_icc_ci(bc.short2, upper.multiplier = 10) 
coxme_icc_ci(bc.short3, upper.multiplier = 10) 

coxme_icc_ci(bl.short, upper.multiplier = 10) 
coxme_icc_ci(bl.short2, upper.multiplier = 10) 
coxme_icc_ci(bl.short3, upper.multiplier = 10) 

coxme_icc_ci(bh.short, upper.multiplier = 10)
coxme_icc_ci(bh.short2, upper.multiplier = 10)
coxme_icc_ci(bh.short3, upper.multiplier = 10)

coxme_icc_ci(bc.long, upper.multiplier = 10)
coxme_icc_ci(bc.long2, upper.multiplier = 10)
coxme_icc_ci(bc.long3, upper.multiplier = 10)

coxme_icc_ci(bl.long, upper.multiplier = 10)
coxme_icc_ci(bl.long2, upper.multiplier = 10)
coxme_icc_ci(bl.long3, upper.multiplier = 10)

coxme_icc_ci(bh.long, upper.multiplier = 10)
coxme_icc_ci(bh.long2, upper.multiplier = 10)
coxme_icc_ci(bh.long3, upper.multiplier = 10)

coxme_pval(bc.short,etho[which(etho$term == "short"),],boot=100)
coxme_pval(bl.short,etho[which(etho$term == "short"),],boot=100)
coxme_pval(bh.short,etho[which(etho$term == "short"),],boot=100)

coxme_pval(bc.long,etho[which(etho$term == "long"),],boot=100) 
coxme_pval(bl.long,etho[which(etho$term == "long"),],boot=100) 
coxme_pval(bh.long,etho[which(etho$term == "long"),],boot=100) 

#activity
library(rptR)
ac.short = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "short" & etho$Treatment == "0"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)
ac.short2 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term2 == "short" & etho$Treatment == "0"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)
ac.short3 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term3 == "short" & etho$Treatment == "0"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)

al.short = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "short" & etho$Treatment == "5"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)
al.short2 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term2 == "short" & etho$Treatment == "5"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)
al.short3 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term3 == "short" & etho$Treatment == "5"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)

ah.short = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "short" & etho$Treatment == "25"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)
ah.short2 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term2 == "short" & etho$Treatment == "25"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)
ah.short3 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term3 == "short" & etho$Treatment == "25"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500)


ac.long = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "long" & etho$Treatment == "0"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
ac2.long = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term2 == "long" & etho$Treatment == "0"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
ac3.long = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term3 == "long" & etho$Treatment == "0"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 

al.long = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "long" & etho$Treatment == "5"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
al.long2 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term2 == "long" & etho$Treatment == "5"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
al.long3 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term3 == "long" & etho$Treatment == "5"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 

ah.long = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term == "long" & etho$Treatment == "25"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
ah.long2 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term2 == "long" & etho$Treatment == "25"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 
ah.long3 = rpt(log(Mean.Mobility+1) ~ scale.age + (1|FishName) + (1|Clutch), datatype = "Gaussian", data=etho[which(etho$term3 == "long" & etho$Treatment == "25"),], grname = c("FishName","Clutch"), nboot = 500, npermut = 500) 


tdata = data.frame("Trait" = c("Exploration","Exploration","Exploration","Exploration","Exploration","Exploration",
                               "Boldness","Boldness","Boldness","Boldness","Boldness","Boldness",
                              "Activity","Activity","Activity","Activity","Activity","Activity"),
                  "Repeatability" = c(0.11, 0.05, 0.03, 0.13, 0.06, 0.26,
                                      0.10, 0, 0, 0.22, 0.17, 0.25,
                                      0, 0.02, 0, 0.30, 0.18, 0.32),
                  "UCI" = c(0.28, 0.19, 0.14, 0.30, 0.20, 0.47, 
                            0.22, 0, 0, 0.37, 0.34, 0.41, 
                            0.12, 0.16, 0.11, 0.47, 0.33, 0.49),
                  "LCI" = c(0, 0, 0, 0, 0, 0.04, 
                            0.01, 0, 0, 0.10, 0.03, 0.12, 
                            0, 0, 0, 0.12, 0.02, 0.16),
                  "Term" = c("Short","Short","Short","Long","Long","Long",
                             "Short","Short","Short","Long","Long","Long",
                             "Short","Short","Short","Long","Long","Long"),
                  "Treatment" = c("control","low","high","control","low","high",
                                  "control","low","high","control","low","high",
                                  "control","low","high","control","low","high")
                  )
tdata$Trait = as.factor(tdata$Trait)
tdata$Term = factor(tdata$Term, levels= c("Short", "Long"))
tdata$Treatment = factor(tdata$Treatment, levels = c("control","low","high"))
tdata$Trait = factor(tdata$Trait, levels = c("Boldness", "Exploration", "Activity"))
cols = c("control"= "red","low" = "#999999","high" = "black")

term.plot = ggplot(tdata, aes(x = Term, y = Repeatability, color = Treatment)) +
  facet_wrap(~Trait) +
  geom_pointrange(aes(ymin = LCI, ymax = UCI), 
                  position = position_dodge2(width = c(0.3,0.3))) +
  ...
  ylab("Repeatability") + xlab("Duration since BMAA exposure")



##### Stats ######
#Bootstrapping - randomly sample 1000 values from predicted distribution of short and long term of each personality trait from the model. Line up samples as 2 vectors, one from short and one from long. What proportion of the time is the short value smaller than the long values? If more than 95% of the time, then it's significantly different.

### exploration
#ec.short, el.short, eh.short, ec.long, el.long, eh.long
#control
rpt.ecshort <- ec.short$VCV[,"FishName"]/(ec.short$VCV[,"FishName"]+ec.short$VCV[,"units"])
rpt.eclong <- ec.long$VCV[,"FishName"]/(ec.long$VCV[,"FishName"]+ec.long$VCV[,"units"])
p1 <- hist(rpt.ecshort)                     
p2 <- hist(rpt.eclong)                     
plot( p1, col=rgb(0,0,1,1/4), xlim=c(0,0.3))  
plot( p2, col=rgb(1,0,0,1/4), xlim=c(0,0.4), add=T)
cont = data.frame(cbind(rpt.ecshort,rpt.eclong))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
1-(1514/2900) # p = 0.48
# Posterior predicted values for exploration, control treatment fish are lower in the short duration 54% of the time
rpt.ecshort2 <- ec.short2$VCV[,"FishName"]/(ec.short2$VCV[,"FishName"]+ec.short2$VCV[,"units"])
rpt.eclong2 <- ec.long2$VCV[,"FishName"]/(ec.long2$VCV[,"FishName"]+ec.long2$VCV[,"units"])
cont = data.frame(cbind(rpt.ecshort2,rpt.eclong2))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff) 
1 - (882/2900) # p = 0.70
rpt.ecshort3 <- ec.short3$VCV[,"FishName"]/(ec.short3$VCV[,"FishName"]+ec.short3$VCV[,"units"])
rpt.eclong3 <- ec.long3$VCV[,"FishName"]/(ec.long3$VCV[,"FishName"]+ec.long3$VCV[,"units"])
cont = data.frame(cbind(rpt.ecshort3,rpt.eclong3))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
1-(710/2900) # p = 0.76
...
#low
rpt.elshort <- el.short$VCV[,"FishName"]/(el.short$VCV[,"FishName"]+el.short$VCV[,"units"])
rpt.ellong <- el.long$VCV[,"FishName"]/(el.long$VCV[,"FishName"]+el.long$VCV[,"units"])
low = data.frame(cbind(rpt.elshort,rpt.ellong))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff)
1-(1484/2900) # p = 0.49
rpt.elshort2 <- el.short2$VCV[,"FishName"]/(el.short2$VCV[,"FishName"]+el.short2$VCV[,"units"])
rpt.ellong2 <- el.long2$VCV[,"FishName"]/(el.long2$VCV[,"FishName"]+el.long2$VCV[,"units"])
low = data.frame(cbind(rpt.elshort2,rpt.ellong2))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff)
1-(1014/2900) # p = 0.65
rpt.elshort3 <- el.short3$VCV[,"FishName"]/(el.short3$VCV[,"FishName"]+el.short3$VCV[,"units"])
rpt.ellong3 <- el.long3$VCV[,"FishName"]/(el.long3$VCV[,"FishName"]+el.long3$VCV[,"units"])
low = data.frame(cbind(rpt.elshort3,rpt.ellong3))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff)
1-(1653/2900) # p = 0.43
...
#high
rpt.ehshort <- eh.short$VCV[,"FishName"]/(eh.short$VCV[,"FishName"]+eh.short$VCV[,"units"])
rpt.ehlong <- eh.long$VCV[,"FishName"]/(eh.long$VCV[,"FishName"]+eh.long$VCV[,"units"])
high = data.frame(cbind(rpt.ehshort,rpt.ehlong))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(2836/2900) # p = 0.02 - short repeatability values significantly lower than long duration values
rpt.ehshort2 <- eh.short2$VCV[,"FishName"]/(eh.short2$VCV[,"FishName"]+eh.short2$VCV[,"units"])
rpt.ehlong2 <- eh.long2$VCV[,"FishName"]/(eh.long2$VCV[,"FishName"]+eh.long2$VCV[,"units"])
high = data.frame(cbind(rpt.ehshort2,rpt.ehlong2))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(2471/2900) # p = 0.15
rpt.ehshort3 <- eh.short3$VCV[,"FishName"]/(eh.short3$VCV[,"FishName"]+eh.short3$VCV[,"units"])
rpt.ehlong3 <- eh.long3$VCV[,"FishName"]/(eh.long3$VCV[,"FishName"]+eh.long3$VCV[,"units"])
high = data.frame(cbind(rpt.ehshort3,rpt.ehlong3))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(902/2900) # p = 0.69

### boldness - comparing predicted values from the coxme fit
# How do rpt values of log latency compare to survival model? R = 0.09, CI = 0.05-0.13
bold = MCMCglmm(log(LatencyZ2360+1) ~ Treatment + scale.age, random =~ FishName, family = "gaussian", 
                   data = etho,
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bold <- bold$VCV[,"FishName"]/(bold$VCV[,"FishName"]+bold$VCV[,"units"]) 
mean(rpt.bold) # R=0.10 
HPDinterval(rpt.bold ) # CI=0.06-0.16
# rpt and CI very similar between Gaussian and survival model. We don't know how to do a randomization test with a survival model, so we will use the Gaussian distribution in a Bayesian framework.

#control
bc.short = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=103000, thin=20, burnin=5000)
rpt.bcshort <- bc.short$VCV[,"FishName"]/(bc.short$VCV[,"FishName"]+bc.short$VCV[,"units"])
bc.long = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=103000, thin=20, burnin=5000)
rpt.bclong <- bc.long$VCV[,"FishName"]/(bc.long$VCV[,"FishName"]+bc.long$VCV[,"units"])
cont = data.frame(cbind(rpt.bcshort,rpt.bclong))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
1-(4815/4900) # p = 0.02

bc.short2 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term2 == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=103000, thin=20, burnin=5000)
rpt.bcshort2 <- bc.short2$VCV[,"FishName"]/(bc.short2$VCV[,"FishName"]+bc.short2$VCV[,"units"])
bc.long2 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term2 == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=103000, thin=20, burnin=5000)
rpt.bclong2 <- bc.long2$VCV[,"FishName"]/(bc.long2$VCV[,"FishName"]+bc.long2$VCV[,"units"])
cont = data.frame(cbind(rpt.bcshort2,rpt.bclong2))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
1-(4431/4900) # p = 0.10

bc.short3 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term3 == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=103000, thin=20, burnin=5000)
rpt.bcshort3 <- bc.short3$VCV[,"FishName"]/(bc.short3$VCV[,"FishName"]+bc.short3$VCV[,"units"])
bc.long3 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term3 == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior, nitt=103000, thin=20, burnin=5000)
rpt.bclong3 <- bc.long3$VCV[,"FishName"]/(bc.long3$VCV[,"FishName"]+bc.long3$VCV[,"units"])
cont = data.frame(cbind(rpt.bcshort3,rpt.bclong3))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
 1-(4631/4900) # p = 0.05
...
#low
bl.short = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.blshort <- bl.short$VCV[,"FishName"]/(bl.short$VCV[,"FishName"]+bl.short$VCV[,"units"]) 
bl.long = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bllong <- bl.long$VCV[,"FishName"]/(bl.long$VCV[,"FishName"]+bl.long$VCV[,"units"]) 
cont = data.frame(cbind(rpt.blshort,rpt.bllong))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
1-(2677/2900) # p = 0.08

bl.short2 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term2 == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.blshort2 <- bl.short2$VCV[,"FishName"]/(bl.short2$VCV[,"FishName"]+bl.short2$VCV[,"units"]) 
bl.long2 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term2 == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bllong2 <- bl.long2$VCV[,"FishName"]/(bl.long2$VCV[,"FishName"]+bl.long2$VCV[,"units"]) 
cont = data.frame(cbind(rpt.blshort2,rpt.bllong2))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
1-(2669/2900) # p = 0.08

bl.short3 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term3 == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.blshort3 <- bl.short3$VCV[,"FishName"]/(bl.short3$VCV[,"FishName"]+bl.short3$VCV[,"units"]) 
bl.long3 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term3 == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bllong3 <- bl.long3$VCV[,"FishName"]/(bl.long3$VCV[,"FishName"]+bl.long3$VCV[,"units"]) 
cont = data.frame(cbind(rpt.blshort3,rpt.bllong3))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff)
1-(2117/2900) # p = 0.27
...
#high
bh.short = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bhshort <- bh.short$VCV[,"FishName"]/(bh.short$VCV[,"FishName"]+bh.short$VCV[,"units"]) 
bh.long = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bhlong <- bh.long$VCV[,"FishName"]/(bh.long$VCV[,"FishName"]+bh.long$VCV[,"units"]) 
high = data.frame(cbind(rpt.bhshort,rpt.bhlong))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(2890/2900) # p = 0.003

bh.short2 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term2 == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bhshort2 <- bh.short2$VCV[,"FishName"]/(bh.short2$VCV[,"FishName"]+bh.short2$VCV[,"units"]) 
bh.long2 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term2 == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bhlong2 <- bh.long2$VCV[,"FishName"]/(bh.long2$VCV[,"FishName"]+bh.long2$VCV[,"units"]) 
high = data.frame(cbind(rpt.bhshort2,rpt.bhlong2))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(2895/2900) # p = 0.002

bh.short3 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term3 == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bhshort3 <- bh.short3$VCV[,"FishName"]/(bh.short3$VCV[,"FishName"]+bh.short3$VCV[,"units"]) 
bh.long3 = MCMCglmm(log(LatencyZ2360+1) ~ scale.age, random =~ FishName, family = "gaussian", 
                   data = etho[which(etho$term3 == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)
rpt.bhlong3 <- bh.long3$VCV[,"FishName"]/(bh.long3$VCV[,"FishName"]+bh.long3$VCV[,"units"]) 
high = data.frame(cbind(rpt.bhshort3,rpt.bhlong3))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(2810/2900) # p = 0.03
...
### activity - use MCMCglmm to get posterior distribution
prior2 = list(R=list(R1=list(V=1,nu=0.002),
                    R2=list(V=1,nu=0.002),
                    R3=list(V=1,nu=0.002)), 
             G=list(G1=list(V=1,nu=0.002),
                    G2=list(V=1,nu=0.002))) #weak priors

#control
ac.short = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.acshort <- ac.short$VCV[,"FishName"]/(ac.short$VCV[,"FishName"]+ac.short$VCV[,"units"]) 
ac.long = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.aclong <- ac.long$VCV[,"FishName"]/(ac.long$VCV[,"FishName"]+ac.long$VCV[,"units"]) 
cont = data.frame(cbind(rpt.acshort,rpt.aclong))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff) 
1-(2897/2900) # p = 0.001

ac.short2 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term2 == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.acshort2 <- ac.short2$VCV[,"FishName"]/(ac.short2$VCV[,"FishName"]+ac.short2$VCV[,"units"]) 
ac.long2 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term2 == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.aclong2 <- ac.long2$VCV[,"FishName"]/(ac.long2$VCV[,"FishName"]+ac.long2$VCV[,"units"]) 
cont = data.frame(cbind(rpt.acshort2,rpt.aclong2))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff) 
1-(2795/2900) # p = 0.04

ac.short3 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term3 == "short" & etho$Treatment == "0"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.acshort3 <- ac.short3$VCV[,"FishName"]/(ac.short3$VCV[,"FishName"]+ac.short3$VCV[,"units"]) 
ac.long3 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term3 == "long" & etho$Treatment == "0"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.aclong3 <- ac.long3$VCV[,"FishName"]/(ac.long3$VCV[,"FishName"]+ac.long3$VCV[,"units"]) 
cont = data.frame(cbind(rpt.acshort3,rpt.aclong3))
colnames(cont) = c("short","long")
cont$diff = ifelse(cont$short < cont$long, 0,1)
table(cont$diff) 
1-(2233/2900) # p > 0.05
...
#low
al.short = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.alshort <- al.short$VCV[,"FishName"]/(al.short$VCV[,"FishName"]+al.short$VCV[,"units"]) 
al.long = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.allong <- al.long$VCV[,"FishName"]/(al.long$VCV[,"FishName"]+al.long$VCV[,"units"]) 
low = data.frame(cbind(rpt.alshort,rpt.allong))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff) 
1-(2727/2900) # p = 0.06

al.short2 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term2 == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.alshort2 <- al.short2$VCV[,"FishName"]/(al.short2$VCV[,"FishName"]+al.short2$VCV[,"units"]) 
al.long2 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term2 == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.allong2 <- al.long2$VCV[,"FishName"]/(al.long2$VCV[,"FishName"]+al.long2$VCV[,"units"]) 
low = data.frame(cbind(rpt.alshort2,rpt.allong2))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff) 
1-(2794/2900) # p = 0.04

al.short3 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term3 == "short" & etho$Treatment == "5"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.alshort3 <- al.short3$VCV[,"FishName"]/(al.short3$VCV[,"FishName"]+al.short3$VCV[,"units"]) 
al.long3 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term3 == "long" & etho$Treatment == "5"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.allong3 <- al.long3$VCV[,"FishName"]/(al.long3$VCV[,"FishName"]+al.long3$VCV[,"units"]) 
low = data.frame(cbind(rpt.alshort3,rpt.allong3))
colnames(low) = c("short","long")
low$diff = ifelse(low$short < low$long, 0,1)
table(low$diff) 
1-(2566/2900) # p = 0.12
...
#high
ah.short = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.ahshort <- ah.short$VCV[,"FishName"]/(ah.short$VCV[,"FishName"]+ah.short$VCV[,"units"]) 
ah.long = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.ahlong <- ah.long$VCV[,"FishName"]/(ah.long$VCV[,"FishName"]+ah.long$VCV[,"units"]) 
high = data.frame(cbind(rpt.ahshort,rpt.ahlong))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(2896/2900) # p = 0.001

ah.short2 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term2 == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.ahshort2 <- ah.short2$VCV[,"FishName"]/(ah.short2$VCV[,"FishName"]+ah.short2$VCV[,"units"]) 
ah.long2 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term2 == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.ahlong2 <- ah.long2$VCV[,"FishName"]/(ah.long2$VCV[,"FishName"]+ah.long2$VCV[,"units"]) 
high = data.frame(cbind(rpt.ahshort2,rpt.ahlong2))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(2885/2900) # p = 0.005

ah.short3 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term3 == "short" & etho$Treatment == "25"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.ahshort3 <- ah.short3$VCV[,"FishName"]/(ah.short3$VCV[,"FishName"]+ah.short3$VCV[,"units"]) 
ah.long3 = MCMCglmm(log(Mean.Mobility+1) ~ scale.age, random =~ FishName + Clutch, family = "gaussian", 
                   data = etho[which(etho$term3 == "long" & etho$Treatment == "25"),],
               verbose=F, prior=prior2, nitt=63000, thin=20, burnin=5000)
rpt.ahlong3 <- ah.long3$VCV[,"FishName"]/(ah.long3$VCV[,"FishName"]+ah.long3$VCV[,"units"]) 
high = data.frame(cbind(rpt.ahshort3,rpt.ahlong3))
colnames(high) = c("short","long")
high$diff = ifelse(high$short < high$long, 0,1)
table(high$diff) 
1-(722/2900) # p > 0.05
...
R is higher when random slope is included. 
...
besyn = ggplot(df_cors, aes(x = Traits, y = Estimate)) +
