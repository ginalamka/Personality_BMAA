...
3. Boldness = Cumulative duration in the center (count or percent) OR Latency to enter the center (see exploration).
...
VarCorr(b2) #Random effects only
...
rptB = rpt(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName) + (1|OLRE), datatype = "Poisson", data=etho, grname = "FishName", nboot = 100, npermut = 100) # model fails to converge with the OLRE 🤦
rptB
...
sum(results > 0.12)/10 # probability the repeatability estimate from our actual data is greater than the repeatability estimates generated from the permutations which randomized the data


#Kelsey, do you expect reviewers may have a problem with using both rptR and MCMCglmm for repeatability values, or should we switch to MCMCglmms for all variables?
...
It is straightforward to extract the random effect variance from the output of a coxme survival model. However, according to this post: https://stat.ethz.ch/pipermail/r-sig-mixed-models/2015q3/023871.html, there is no residual variance in Cox survival models.



## Activity
Mobility is our measure of activity. This variable describes the percentage of pixel change between the current sample at the time (t) and previous sample (t-1) *this is detected in the subject only, not the full arena* (aka "activity" described by EthoVision) ; ranges from 0 - 100%. Mobility depends on the size of subject only, not on the arena size; a small fish results in a small number of pixels that change, therefore small movements may result in high change.
...
Would there be a benefit to test for a behavioral syndrome here? i.e., is there a relationship among the three behavioral parameters?

something to consider: will having both rptR and MCMCglmm calculations affect behav synd tests? assume it is ok since we'll test the model, not the repeatability output
