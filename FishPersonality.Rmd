...
#Binomial:
simulationOutput <- simulateResiduals(fittedModel = glmer(cbind(round(CumDurZ2),trial.time) ~ Treatment + scale(Age, center = T, scale = T) + Treatment*scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity), family = binomial, data=etho), n=250) 
#Poisson with Observation-level random effect to account for overdispersion:
simulationOutput <- simulateResiduals(fittedModel = glmer(round(CumDurZ2) ~ Treatment + scale(Age, center = T, scale = T) + (1|FishName) +  (1|OLRE), family = poisson, data=etho), n=500) #model failed to converge with interaction effect. isSingular warning because of Paternity & Clutch
...
b1 = glmer(round(CumDurZ2) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName) + (1|Clutch) + (1|Paternity) + (1|OLRE), family = poisson, data=etho) # full model
...
b1 = glmer(cbind(round(CumDurZ2),trial.time) ~ Treatment + scale(Age, center = T, scale = T) + Treatment*scale(Age, center = T, scale = T) + (1|FishName) + (1|Clutch) + (1|Paternity), family = binomial, data=etho)
...
b2 = glmer(round(CumDurZ2) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName) + (1|OLRE), family = poisson, data=etho) #model fails to converge with Paternity included, and it accounts for little variance
anova(b1,b2) # no difference, proceed with model 2

b3 = glmer(round(CumDurZ2) ~ Treatment + scale.age + (1|FishName) + (1|OLRE), family = poisson, data=etho) # no interaction
...
rptB = rpt(round(CumDurZ2) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName) + (1|OLRE), datatype = "Poisson", data=etho, grname = "FishName", nboot = 100, npermut = 100) # model fails to converge with the OLRE 🤦
rptB
