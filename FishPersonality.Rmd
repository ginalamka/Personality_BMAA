...
plot(mlt.var) #clearly some autocorrelation, so need to run more iterations still, but so far all looks pretty good
...
HPDinterval(cor_MobDur) # -0.23-0.40 ...crosses zero so non-significant correlation
plot(cor_MobDur)
...
mean(cor_MobLat) # -0.76
HPDinterval(cor_MobLat) # -0.94 - -0.59 ...does NOT cross zero, so significant correlation
plot(cor_MobLat)
...
(sqrt(mlt.var$VCV[,"traitLatencyZ2360:traitLatencyZ2360.FishName"])*
sqrt(mlt.var$VCV[,"traitCumDur.Z2:traitCumDur.Z2.FishName"]))
mean(cor_LatDur) # -0.36
HPDinterval(cor_LatDur) # -0.67 - -0.11 ...does NOT cross zero, so significant correlation
plot(cor_LatDur)
