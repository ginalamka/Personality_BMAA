...
rptB = rpt(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age + (1|FishName), datatype = "Poisson", data=etho, grname = "FishName", nboot = 100, npermut = 100) # Poisson models are automatically fit with an OLRE in the rptR package.
rptB # R = 0.10 but se = 0 and CI is 0.10-0.10 so something is wrong here.
...
summary(m4)
levels(etho$Treatment)
library(nlme) # use this package to get significance values for fixed effects
...
## Treatment25 B = 0.11, p = 0.05; Compared to control fish, treatment 25 fish are significantly more active
## Age B = -0.10, p = 0; As age/trial number increases, activity level decreases

# does it matter if we re-scale Treatment factor levels? NO, effect sizes and sign directions from model are the same
etho$Treatment = factor(etho$Treatment, levels = c("0","25","5"))
etho$Treatment = factor(etho$Treatment, levels = c("0","5","25"))
...
```
Treatment significantly relates to activity level, but the relationship varies by amount of treatment. Treatment 5 fish are less active, while Treatment 25 fish are more active relative to Control fish.
