...
coxme_icc_ci(bc.short, upper.multiplier = 10) # ICC = 
coxme_icc_ci(bl.short, upper.multiplier = 10) # ICC = 
coxme_icc_ci(bh.short, upper.multiplier = 10) # ICC = 

coxme_icc_ci(bc.long, upper.multiplier = 10)
coxme_icc_ci(bl.long, upper.multiplier = 10)
coxme_icc_ci(bh.long, upper.multiplier = 10)

coxme_pval(bc.short,etho[which(etho$term == "short"),],boot=100)#  p
coxme_pval(bl.short,etho[which(etho$term == "short"),],boot=100)#  p
coxme_pval(bh.short,etho[which(etho$term == "short"),],boot=100)#  p

coxme_pval(bc.long,etho[which(etho$term == "long"),],boot=100) # p 
coxme_pval(bl.long,etho[which(etho$term == "long"),],boot=100) # p
coxme_pval(bh.long,etho[which(etho$term == "long"),],boot=100) # p
...
term.plot = ggplot(tdata, aes(x = Trait, y = Repeatability, color = Treatment)) +
  facet_wrap(~Term) +
  geom_pointrange(aes(ymin = LCI, ymax = UCI), 
...
#Bootstrapping - randomly sample 1000 values from predicted distribution of short and long term of each personality trait from the model. Line up samples as 2 vectors, one from short and one from long. What proportion of the time is the short value smaller than the long values? If more than 95% of the time, then it's significantly different.
  ...
  e.short <- MCMCglmm(round(CumDur.Z2) ~ Treatment + scale.age + Treatment*scale.age, random = ~FishName, family = "poisson", data = tmp,
               verbose=F, prior=prior, nitt=63000, thin=20, burnin=5000)

rptFunc = function(x){
  stat = x$VCV[,"FishName"]/(x$VCV[,"FishName"]+x$VCV[,"units"])
  return(stat)
}

test = bootMer(a.short, FUN=rptFunc, nsim=2)
########
...
cont = data.frame(cbind(sort(rpt.acshort),sort(rpt.aclong)))
